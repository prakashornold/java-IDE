...
export function InterviewPrep({ onNavigateHome, onNavigateToAdmin }: InterviewPrepProps) {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-[#1a1b22]">
      ...
      <div className="flex-shrink-0 bg-[#1a1b22] border-b border-[#282934]">
        <div className="absolute left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#5294d0]/20 to-transparent" style={{ top: 'auto' }} />
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-4 pb-0">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-[#4080b5] to-[#2a5580] flex items-center justify-center shadow-sm shadow-[#3a6d9e]/20 flex-shrink-0">
                  <BookOpen className="w-3 h-3 text-white" strokeWidth={2.5} />
                </div>
                <h1 className="text-sm font-bold text-[#f1f3f5]">Interview Preparation</h1>
              </div>
            </div>

            <p className="text-[11px] text-[#5d6472] hidden sm:block">
              {totalQuestions} questions &middot; {categoryKeys.length} topics
            </p>
          </div>

          <div className="relative mb-3">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#5d6472] pointer-events-none" />
            <input
              type="text"
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              placeholder="Search questions..."
              className="w-full pl-9 pr-9 py-2 bg-[#13141a] border border-[#282934] rounded-lg text-xs text-[#d5d9e0] placeholder-[#5d6472] focus:outline-none focus:border-[#5294d0]/50 focus:ring-1 focus:ring-[#5294d0]/20 transition-all"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-[#5d6472] hover:text-[#9ba1ad] transition-colors"
              >
          ...
          {!isSearching && (
            <div className="flex items-center gap-1.5 flex-wrap pb-3 overflow-x-auto scrollbar-none">
              <button
                onClick={() => setActiveCategory('ALL')}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-150 flex-shrink-0 ${
                  activeCategory === 'ALL'
              ...
              {categoryKeys.map(key => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-150 border flex-shrink-0 ${
                      isActive
                  ...
                  >
                    ...
                    <span className="sm:hidden">{cat.label.split(/[\s&]/)[0]}</span>
                  </button>
                );
              })}
            </div>

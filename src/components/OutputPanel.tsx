    ...
    <div className="flex flex-col h-full bg-[#1a1b22]">
      <div className="flex items-center justify-between px-3.5 sm:px-4 py-2.5 border-b border-[#282934] bg-[#1e1f26]">
        <div className="flex items-center gap-2">
          <Terminal className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-[#585d6a]" />
          <span className="text-xs sm:text-sm font-semibold text-[#c8ccd4] tracking-tight">Input / Output</span>
        </div>

        <div className="flex items-center gap-2 sm:gap-2.5">
          {isRunning && (
            <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs px-2.5 py-1 rounded-md bg-[#5294d0]/10 border border-[#5294d0]/20 animate-border-glow">
          ...
          {!isRunning && output && !hasError && (
            <div className="flex items-center gap-1.5 text-[10px] sm:text-xs px-2.5 py-1 rounded-md bg-[#6aab73]/10 border border-[#6aab73]/20">
          ...
          {!isRunning && hasError && (
            <div className="flex items-center gap-1.5 text-[10px] sm:text-xs px-2.5 py-1 rounded-md bg-[#cf6679]/10 border border-[#cf6679]/20">
          ...
          {!isMobile && (
            ...
            >
              {layoutMode === 'bottom' ? (
                <PanelRightOpen className="w-4 h-4 text-[#585d6a] group-hover:text-[#c8ccd4] transition-colors" />
              ) : (
                <PanelBottomOpen className="w-4 h-4 text-[#585d6a] group-hover:text-[#c8ccd4] transition-colors" />
              )}
            </button>
          )}
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex flex-col border-b border-[#282934]" style={{ height: '15%', minHeight: '60px' }}>
          <div className="flex items-center gap-2 px-3.5 sm:px-4 py-1.5 bg-[#1e1f26]/60 border-b border-[#282934]/60">
            <Keyboard className="w-3 sm:w-3.5 h-3 sm:h-3.5 text-[#585d6a]" />
            <span className="text-[10px] sm:text-xs font-medium text-[#848996]">Input (stdin)</span>
          </div>
          <textarea
            value={stdin}
            onChange={(e) => onStdinChange(e.target.value)}
            placeholder="Enter program input here (one value per line)..."
            className="flex-1 w-full px-3.5 sm:px-4 py-2 bg-[#1a1b22] text-[#c8ccd4] font-mono text-xs sm:text-sm resize-none focus:outline-none placeholder:text-[#3e4250] transition-colors"
            spellCheck={false}
          />
        </div>
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <div className="flex items-center gap-2 px-3.5 sm:px-4 py-1.5 bg-[#1e1f26]/60 border-b border-[#282934]/60">
            <Terminal className="w-3 sm:w-3.5 h-3 sm:h-3.5 text-[#585d6a]" />
            <span className="text-[10px] sm:text-xs font-medium text-[#848996]">Output</span>
          </div>
            ...
            {isRunning ? (
              <div className="flex items-center justify-center h-full">
                <div className="flex flex-col items-center gap-4">
                  ...
                  <p className="text-xs sm:text-sm text-center px-4 font-medium text-[#848996]">Compiling and running your code...</p>
                </div>
              </div>
            ) : output ? (
              <pre
              ...
              <div className="flex items-center justify-center h-full">
                <div className="text-center px-4 py-6">
                  <div className="w-10 h-10 mx-auto mb-3 rounded-lg bg-[#25262f] flex items-center justify-center border border-[#282934]">
                    <Code2 className="w-5 h-5 text-[#585d6a]" />
                  </div>
                  <p className="text-xs sm:text-sm font-medium text-[#585d6a] mb-2">
                    Ready to execute
                  </p>
                  <p className="text-[11px] text-[#3e4250]">
                    Click <span className="text-[#585d6a] font-medium">Run</span>

...
export function TopicSection({ label, questions, accentColor, defaultOpen = false }: TopicSectionProps) {
  ...
  return (
    <div className="border border-[#282934] rounded-xl overflow-hidden transition-all duration-200 hover:border-[#383946]">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center gap-3 px-4 py-3.5 text-left bg-[#1e1f26] hover:bg-[#25262f] transition-colors duration-150"
      >
        <Hash className="w-3.5 h-3.5 flex-shrink-0" style={{ color: accentColor }} />
        <span className="flex-1 text-sm font-semibold text-[#d5d9e0]">{label}</span>

    ...
    <>
      ...
      <div className="w-full bg-[#1a1b22] flex flex-col h-full md:relative fixed left-0 top-0 z-50 md:z-auto md:w-full" style={{ width: isMobile ? '16rem' : '100%' }}>
        <div className="flex items-center gap-2 px-3.5 py-2.5 border-b border-[#282934] bg-[#1e1f26]">
          ...
          <span className="ml-auto text-[10px] text-[#848996] font-semibold bg-[#25262f] px-2 py-0.5 rounded-md border border-[#282934]">{problems.length}</span>
        </div>
                ...
                <div key={category} className="border-b border-[#282934]/50">
                  ...
                  >
                    {isCategoryExpanded ? (
                      ...
                      <Folder className="w-3.5 h-3.5 text-[#585d6a] group-hover:text-[#848996] flex-shrink-0 transition-colors" />
                    )}
                    <span className={`text-xs font-bold tracking-tight truncate ${isCategoryExpanded ? 'text-[#e8eaed]' : 'text-[#c8ccd4]'}`}>
                      {category}
                    </span>
                    <span className="text-[10px] text-[#585d6a] ml-auto font-medium flex-shrink-0">{totalProblems}</span>
                    {isCategoryExpanded ? (
                      <ChevronDown className="w-3 h-3 text-[#585d6a] flex-shrink-0" />
                    ) : (
                      <ChevronRight className="w-3 h-3 text-[#585d6a] flex-shrink-0" />
                    )}
                  </button>
                                    ...
                                    .map((problem) => (
                                      ...
                                      >
                                        <div className="flex items-start gap-2">
                                          <span className="text-[10px] text-[#585d6a] flex-shrink-0 mt-0.5 font-mono tabular-nums">
                                            {problem.number}
                                          </span>
                                          <span
